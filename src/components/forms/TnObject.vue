      ...
      <div class="flex flex-col gap-1">
        <IconField :class="{
          ...
          <InputText :class="{
                     ...
                     :model-value="selectedOption?.[props.display as never]"
...
const options = computed(() => {
  ...
  if (selected.value && selectedOption.value) {
    if (r.some(e => (e as never)[props.fetchValueKey as never] === selected.value)) return r;
...
const getOption = (value?: unknown) => {
  return options.value.find(opt => (opt as never)[props.fetchValueKey as never] === value);
};
